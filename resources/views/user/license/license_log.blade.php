    ...
    <div class="container">
        <div class="row mb-5">
            <div class="col-12 d-flex justify-content-between align-items-center flex-column flex-md-row ">
                <h5 class="i_t mb-0">License Logs</h5>
        ...
        <div class="row">
            <div class="col-12">
                <div class="date_wrapr">
                    <span>Sort By: </span>
                    <form action="{{ route('license.logs') }}" method="get">
                        <div class="row">
            ...
            <div class="col-12 p-0">
                ...
                <div class="maain-tabble table-responsive">
                    <table class="table zero-configuration pay_logs">
                        ...
                        <tbody>
                            @foreach ($license_log as $key => $payments)

                                @php
                                $license_logs = $payments->license
                                @endphp
                            <tr>
                                <td>{{ $key++ }}</td>
                                <td>{{ $license_logs->id }}</td>
                                <td>{{ \Carbon\Carbon::parse($payments->created_at)->format('M d, Y') }}</td>
                                <td>{{ $license_logs->category->name }}</td>
                                <td>${{ $license_logs->attempts }}</td>
                                <td>{{ $license_logs->fee }}</td>

                                <td>
                                    <button type="button" class="btn  btn-drop-table btn-sm" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> <i class="fa fa-ellipsis-v"></i></button>
                                    <div class="dropdown-menu table_menu">
                                         <a class="dropdown-item" href="{{ route('license.booked',$license_logs->id) }}"><i class="fa fa-eye"></i>View Details </a>
                                    </div>
                                </td>
